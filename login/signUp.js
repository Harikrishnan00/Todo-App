...
let logUserName=document.querySelector("#logInUserName")
let logPass=document.querySelector("#logInPass")
let login=document.querySelector("#login")

login.addEventListener("click",addData)
function addData(){
    firebase.firestore().collection("users").get().then(snapshot=>{
        snapshot.forEach(doc=>{
           if(logUserName.value===doc.id){
               localStorage.setItem('userName', JSON.stringify(doc.id))
               if(logPass.value===doc.data().password){ 
